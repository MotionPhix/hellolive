      ...
      <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:px-6">
          <div class="flex justify-between items-center">
            ...
            <a href="{{ route('contact', ['billboard' => $billboard->id]) }}"
  ...
  @push('scripts')
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        const map = L.map('map').setView([{{ $billboard->latitude }}, {{ $billboard->longitude }}], 15);
        ...
        L.marker([{{ $billboard->latitude }}, {{ $billboard->longitude }}])
          .addTo(map)
          .bindPopup("{{ $billboard->name }}<br>{{ $billboard->location }}");
      });
    </script>
  @endpush
@endsection
