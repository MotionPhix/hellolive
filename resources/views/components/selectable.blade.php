...
<div x-data="{
  ...
  <span x-text="selectedItem ? selectedItem.option : '{{ $placeholder }}'" class="truncate">
    {{ $placeholder }}
  </span>
  ...
  class="absolute w-full py-1 z-10 mt-1 overflow-auto text-sm bg-white rounded-md shadow-md max-h-56 ring-1 ring-black ring-opacity-5 focus:outline-none"
  x-cloak>
  ...
  <svg x-show="selectedItem.id==item.id" class="absolute left-0 w-4 h-4 ml-2 stroke-current text-neutral-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
